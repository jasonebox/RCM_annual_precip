...
#load RACMO to CARRA grid resampling key
fn=tool_path+'resampling_key_RACMO_to_CARRA.pkl'
infile = open(fn,'rb')
df_res=pickle.load(infile) #upsampling

fn=tool_path+'resampling_key_RACMO1km_to_CARRA.pkl'
infile = open(fn,'rb')
df_res_ds=pickle.load(infile) #down-sampling


#define upscaling RACMO
...
fn=raw_path+'precip.1958-2020.BN_RACMO2.3p2_ERA5_3h_FGRN055.1km.YY.nc.gz'
with gzip.open(fn) as gz:
    ds=xr.open_dataset(gz,decode_times=False)

tp_R=np.array(ds.precipcorr[:,:,:]) #total precip in mmWE

fn=raw_path+'snowfall.1958-2020.BN_RACMO2.3p2_ERA5_3h_FGRN055.1km.YY.nc.gz'
with gzip.open(fn) as gz:
    ds=xr.open_dataset(gz,decode_times=False)
sf_R=np.array(ds.snowfallcorr[0:63,:,:]) #snowfall in mmWE
...
for j, year in enumerate(years):
    tpp=tp_R[j]
    tpp=upscaleRACMOtoCARRA(tpp)
    tpp[maskC==0]=np.nan
    tp_5820_C[j,:,:]=tpp
        
    sff=sf_R[j]
    sff=upscaleRACMOtoCARRA(sff)
    sff[maskC==0]=np.nan
    sf_5820_C[j,:,:]=sff
...
df_out.to_csv(path+'RCM_annual_precip/RACMO1km_1958to2020_yearly.csv', index=False)
