...
def NHMtoCARRA(tpp, df_res, niC, njC):
    tp_res= tpp[df_res.row_n, df_res.col_n]
    tp_res=tp_res.reshape(niC, njC)
    return tp_res

#load data

fn=raw_path+'NHM-SMAP_v1_inter-annual_8020.nc'
ds=xr.open_dataset(fn)

tp_N=np.sum(np.array(ds.PRECIP[:,:,:]), axis=1) #rainfall in mmWE
sf_N=np.sum(np.array(ds.SNOW[:,:,:]), axis=1) #snowfall in mmWE
...
for j, year in enumerate(years):
    tpp=tp_N[j]
    tpp=NHMtoCARRA(tpp, df_res, niC, njC)
    tpp[maskC==0]=np.nan
    tp_8020_C[j,:,:]=tpp
        
    sff=sf_N[j]
    sff=NHMtoCARRA(sff, df_res, niC, njC)
    sff[maskC==0]=np.nan
    sf_8020_C[j,:,:]=sff
...
df_out.to_csv(path+'RCM_annual_precip/NHM_1980to2020_yearly_tp.csv', index=False)
