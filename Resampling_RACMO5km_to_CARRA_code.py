...
with gzip.open(fn) as gz:
    ...
    #load RACMO dataset
    RACMO_data = np.array(ds.snowfall[0, 0, :, :])
    lon=ds.lon.values
    lat=ds.lat.values
...
x2, y2 = lon.flatten(), lat.flatten()
rx, ry = transform(inProj, outProj, x2, y2)
...
cols2, rows2 = np.meshgrid(np.arange(njR), np.arange(niR))
...
CARRA_cells_for_RACMO.to_pickle(path_tools+'resampling_key_RACMO5km_to_CARRA.pkl')
